...
if (isset($_REQUEST['testid']) and ($_REQUEST['testid'] > 0)) {
    ...
    $out_filename = 'smartexam_questions_'.$test_id.'_'.date('YmdHis');
...
function F_xml_export_question_stats($test_id)
{
    ...
    $xml .= '<smartexamquestionstats version="'.K_TCEXAM_VERSION.'">'.K_NEWLINE;
    $xml .=  K_TAB.'<header';
    ...
    $xml .= K_TAB.'</body>'.K_NEWLINE;
    $xml .= '</smartexamquestionstats>'.K_NEWLINE;
